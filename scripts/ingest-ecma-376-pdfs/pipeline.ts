 ...
 *
 * Usage:
 *   bun scripts/ingest-ecma-376-pdfs/pipeline.ts <part-number> <pdf-path>
 *
 ...
 *
 * Example:
 *   bun scripts/ingest-ecma-376-pdfs/pipeline.ts 1 ./pdfs/ECMA-376-Part1.pdf
 */
...
async function main() {
	...
	if (args.length < 2) {
		console.log("Usage: bun scripts/ingest-ecma-376-pdfs/pipeline.ts <part-number> <pdf-path>");
		console.log("");
		...
		console.log("");
		console.log("Example:");
		console.log("  bun scripts/ingest-ecma-376-pdfs/pipeline.ts 1 ./pdfs/ECMA-376-Part1.pdf");
		process.exit(1);
	}
	...
	for (const pythonPath of pythonPaths) {
		try {
			...
			await $`${pythonPath} scripts/ingest-ecma-376-pdfs/extract.py ${pdfPath} ${extractedDir}`;
			extractSuccess = true;
			break;
		} catch {
	...
	console.log("-".repeat(40));
	await $`bun scripts/ingest-ecma-376-pdfs/chunk.ts ${extractedDir} ${chunksFile}`;

	// Step 3: Embed
	console.log("\n[3/4] Generating embeddings...");
	console.log("-".repeat(40));
	await $`bun scripts/ingest-ecma-376-pdfs/embed.ts ${chunksFile} ${embeddedFile}`;

	// Step 4: Upload
	console.log("\n[4/4] Uploading to database...");
	console.log("-".repeat(40));
	await $`bun scripts/ingest-ecma-376-pdfs/upload.ts ${partNumber} ${embeddedFile}`;
